...
export class TokensService {
  ...
  async getManagedUserToken(username: string): Promise<{ accessToken: string; expiresAt: string }> {
    ...
    try {
      // 1. First, try to create or get a managed user
      const managedUser = await this.createOrGetManagedUser(username);
      
      // 2. Get access token for the managed user
      let tokenResponse;
      if (managedUser.accessToken) {
        // Fresh user creation includes access token (development approach)
        tokenResponse = {
